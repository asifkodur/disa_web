...
class Mylibrary
{
        ...
        // called when student_register form/function is submitted
        {
                if ($this->validate_registration_form() == FALSE) {
                        $this->CI->session->set_flashdata('error', 'Data entered could not be validated');
                } else {
                        //The name of array elements is the same name in dtabase fields
                        $list = array(
                                'name' => $this->CI->input->post('name', TRUE), //TRUE for xss clean
                                'gender' => $this->CI->input->post('gender', TRUE),
                                'dob' => $this->convert_date_to_yyyymmdd($this->CI->input->post('dob', TRUE)), // function converts date to mysqlcompatible format
                                'email' => $this->CI->input->post('u_email', TRUE),
                                'father' => $this->CI->input->post('father', TRUE),
                                'mother' => $this->CI->input->post('mother', TRUE),
                                'address1' => $this->CI->input->post('address1', TRUE),
                                'address2' => $this->CI->input->post('address2', TRUE),
                                'phone' => $this->CI->input->post('phone', TRUE),
                                'course_name' => $this->CI->input->post('course', TRUE),
                                'course_details' => $this->CI->input->post('course_details', TRUE),
                                'institution' => $this->CI->input->post('institution', TRUE),
                                'course_duration_year' => $this->CI->input->post('duration_year', TRUE),
                                'course_duration_month' => $this->CI->input->post('duration_month', TRUE),
                                'financial_status' => $this->CI->input->post('finance', TRUE),
                                'if_working' => $this->CI->input->post('if_working', TRUE),
                                'working_details' => $this->CI->input->post('working_type', TRUE),
                                'interest' => $this->CI->input->post('interests', TRUE),
                                'career_aspiration' => $this->CI->input->post('aspiration', TRUE)
                        );
                        if ($this->CI->database_operations->insert_student($list)) // if operation success returns TRUE
                                {
                                //$this->send_mail();
                                $this->CI->session->set_flashdata('success', 'Successfully Saved Data');
                        } // flash data is displayed in view/register/header.php
                        else {
                                $this->CI->session->set_flashdata('error', 'Something went wrong. Could not Save');
                        } // flash data is displayed in view/register/header.php}
                }
        ...
        public function validate_registration_form()
        {
                ...
                $this->CI->form_validation->set_rules('u_email', 'Email', 'required');
                $this->CI->form_validation->set_rules('dob', "Date of Birth", 'required');
                $this->CI->form_validation->set_rules('father', "Father's Name", 'required');
                $this->CI->form_validation->set_rules('mother', "Mother's Name", 'required');
                $this->CI->form_validation->set_rules('address1', "Address1", 'required');
                $this->CI->form_validation->set_rules('address2', "Address2", 'required');
                $this->CI->form_validation->set_rules('phone', "Phone", 'required');
                $this->CI->form_validation->set_rules('course', "Course", 'required');
                $this->CI->form_validation->set_rules('course_details', "Course Deatils", 'required');
                $this->CI->form_validation->set_rules('duration_year', "Course Duration Year", 'required');
                $this->CI->form_validation->set_rules('duration_month', "Course Duration Month", 'required');
                $this->CI->form_validation->set_rules('finance', "Financial Status", 'required');
                $this->CI->form_validation->set_rules('if_working', "Are you working", 'required');
        ...
        {
                if ($this->CI->session->userdata('user_type') != 'root') {
                        redirect('student_list');
                }
                if ($id != '' and $email != '') {
                        $id    = strip_tags(trim($id));
                        $email = strip_tags(trim(urldecode($email)));
                        if ($this->CI->database_operations->delete_student(array(
                                'student_id' => $id,
                                'email' => $email
                        ))) // gets TRUE on successful delettion
                                {
                                $this->CI->session->set_flashdata('success', 'Successfully Deleted Data');
                        } // flash data is displayed in view/register/header.php
                        else {
                                $this->CI->session->set_flashdata('error', 'Something went wrong. Could not Delete');
                        } // flash data is displayed in view/register/header.php}
                } else {
                        $this->CI->session->set_flashdata('error', 'Something went wrong. Could not Delete');
                }
        ...
        //collects user agent info(requierd for identifying chrome browser android
        {
                ...
                if ($this->CI->agent->is_browser()) {
                        $agent = $agent . $this->CI->agent->browser() . ' ' . $this->CI->agent->version();
                }
                if ($this->CI->agent->is_robot()) {
                        $agent = $agent . $this->CI->agent->robot();
                }
                if ($this->CI->agent->is_mobile()) {
                        $agent = $agent . $this->CI->agent->mobile();
                }
                return $agent;
        }
}
